...
class DB_Manager:
    ...
    def insert_project(self, data):
        if len(data[0]) == 5:
            sql = sql = """INSERT INTO projects (user_id, project_name, url, status_id, description) values(?, ?, ?, ?, ?)"""
            self.__executemany(sql, data)


        if len(data[0]) == 4:
            sql = sql = """INSERT INTO projects (user_id, project_name, url, status_id) values(?, ?, ?, ?)"""
            self.__executemany(sql, data)
...
"""
    ...
    def update_projects(self, param, data):
        sql = sql = f"""UPDATE projects SET {param} = ? WHERE project_name = ? AND user_id = ?"""
        self.__executemany(sql, [data]) 
...
if __name__ == '__main__':
    ...
    #manager.insert_project([(334,'my_project','123132.com.tr',2)])
    #manager.insert_project([(876,'your_project','654546.com.tr',3, "hiii!!! How aree you?")])
    #manager.insert_skill(876, 'your_project', 'API')
